...
#open data file
with open("Log_Volt_vers3.csv","r") as i:
...
#define the function - 
#the model to be examined, where x represents the independent variable values and V (volts) and RC (resistivity) are the parameters to be estimated. This is the linear model with two parameters.
def func(x,V,RC):
    return V - (x/RC)

#The initial guesses of the model parameters
p0=[3.0,50.0] #The first value is the log(voltage) guess, the second value for the guess for resistivity.
...
plt.ylabel("$log_{e}(V)$", fontsize=18)
plt.rc('xtick', labelsize=14) 
plt.rc('ytick', labelsize=14)
plt.errorbar(xdata, ydata, yerr=error, fmt='.k', capsize = 5)
plt.title("Log voltage decay over time", fontsize = 16)
...
#print result values
print()
print("The calculated log initial voltage and S.D. are: ", rans_b, ",",rans_b_SD)
...
fig.savefig("Volt_time.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("Volt_time.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
