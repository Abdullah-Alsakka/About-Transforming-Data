...
the 18 TRGB data, from W. Freedman et al. 2019, 'The Carnegie-Chicago Hubble Program. VIII.'
ApJ vol. 882, 34. The standard (LCDM) model used here requires numerical integration with 3 parameters, 
the Hubble constant, Hu and the normalised matter density, O_m and O_L the cosmological constant
but with presumed elliptical space geometry. An estimate of the normalized space parameter, O_k, 
is the remainder.
"""
...
print("quasi-Euclidean space geometry, sinn(x) = sinh(x).")
...
def func3(x,Hu,O_m,O_L):
    return ((litesped*(1+x))/(Hu*np.sqrt(abs(1-O_m-O_L))))*np.sinh(np.sqrt(abs(1-O_m-O_L))*(func2(x,O_m,O_L)))
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m,ans_O_L), color = "orange", label = "3Psinh$D_L$standard model")
...
fig.savefig("3PsinhDLstandard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("3PsinhDLstandard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
