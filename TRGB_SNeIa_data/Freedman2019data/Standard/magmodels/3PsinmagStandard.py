...
This curve_fit regression routine of Python scipy, uses the data, as mag vs redshift (z) from 
Brout et al. 2022, 'The Pantheon+ Analysis: Cosmological Constraints' Astrophys. J. vol. 938, 110 and 
the TRGB data, from W. Freedman et al. 2019, 'The Carnegie-Chicago Hubble Program. VIII.' ApJ. vol. 882, 34. 
This variation of the LCDM model has three free parameters: Hubble constant, Hu, normalised 
matter density, O_m; the cosmological constant, O_L, with 1=O_m+O_L+O_k. With O_k as the remainder of 
information.

"""
print()
print("This is the 3PmagStandard model, typically known as the standard model of cosmology.")
print("The correlation is distance mag vs. redshift, z, with sinn(x) = sin(x) presuming hyperbolic geometry")
...
O_L = 0.69 # where 1- O_m - O_L is the guesstimate for O_k
...
def func3(x,Hu,O_m,O_L):
    return 5*(np.log10((litesped*(1+x)/(np.sqrt(np.abs(1-O_m-O_L))*Hu))*np.sin((np.sqrt(np.abs(1-O_m-O_L))*func2(x,O_m,O_L))))) + 25
...
# normalised chisquar is calculated for N=1701 SNe Ia + 18 TRGB data pairs with P the parameter count (3) as
P=3
N=1719
e=2.71828183
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m,ans_O_L), color = "green", label = "3PsinmagStandard model")
plt.xlabel("Redshift z", fontsize = 18)
plt.ylabel("$\mu$ (logDistance)", fontsize = 18)
...
fig.savefig("3PmagsinStandard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("3PmagsinStandard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
