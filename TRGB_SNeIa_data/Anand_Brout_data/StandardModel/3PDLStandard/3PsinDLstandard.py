...
An estimate of \Omega_k is possible allowing sinn(x) = sin(x) condition is used presuming 
hyperbolic space geometry.
"""
print()
print("The correlation is luminosity distance, D_L vs. expansion factor with sinn(x) = sin(x) presuming hyperbolic geometry.")
print("")
...
def func3(x,Hu,O_m,O_k):
    return (litesped/(x*Hu*np.sqrt(np.abs(O_k))))*np.sin((np.sqrt(np.abs(O_k)))*(func2(x,O_m,O_k)))
...
Rans_O_L = round(1 - ans_O_m - ans_O_k,3)
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m,ans_O_k), color = "orange", label = "3Psin$D_L$standard model")
...
fig.savefig("3PsinDLstandard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("3PsinDLstandard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
