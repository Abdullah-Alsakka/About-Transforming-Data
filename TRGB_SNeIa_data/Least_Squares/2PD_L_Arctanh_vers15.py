...
This uses the arctanh model, Eqs. 16 and 18 of the manuscript, to calculate H_0, matter density and model fitness.
@author: Mike
"""
print()
print("This least_squares regression routine of Python scipy, uses the 'distance mag' data, converted to luminosity distances, D_L, vs redshift, z, from Brout et al. 2022, 'The Pantheon+ Analysis: Cosmological Constraints' Astrophys. J. vol. 938, 110 and the 18 TRGB data from W. Freedman, 2019 'The Carnegie-Chicago Hubble Program. VIII.' ApJ, vol. 883, 34. The model used is the arctanh analytical solution with two parameters, the Hubble constant, Hu, and the normalized matter density (\u03A9m,O_m) but no value is calculated for dark energy (\u03A9\u039B). The Python 3 least_squares robust regression specifies the loss='cauchy' or 'linear' routine. Integration is unnecessary because the FLRW model can be solved exactly using D_L (as the ydata) vs z.")
...
#Hu represents the Hubble constant and O_m represents the normalized matter density and O_k the spacetime parameter. Here Hu, O_m and O_k are the initial guesses, to be replaced later in this program by computer estimated values. 
Hu=70.0
O_m = 0.0001
O_k = 0.98
#Note the initial guesses cannot sum to be precidely 1, otherwise the Python 3 regression fails.
...
#    O_k = params[2] #the 3 lines to identify 3 parameters
    residual = ydata-((litesped*(1+x)/(Hu*np.sqrt(abs(1-O_m))))*np.sinh(2*(np.arctanh(np.sqrt(abs(1-O_m)))-np.arctanh(np.sqrt(abs(1-O_m))/np.sqrt((O_m*(1+x))+ (1-O_m))))))
    return residual

params=[70,0.001] #Initial guesses: Hubble constant value and normalized matter density: the least_squares regression routine used; x0, the parameter guesses; jac, the math routine used; bounds, allowed lower, then upper bounds for params; loss='cauchy', treatment of outlier data pairs; args, x and y data. 
result2 = least_squares(func1, x0=params, jac='3-point',bounds=((50,0.00001),(80,1.00)),loss='linear',args=(x, ydata))

#presenting the Hu, O_m parameters and rounding the values
Hu,O_m = result2.x
...
yfit1 = ((litesped*(1+x)/(Hu*np.sqrt(abs(1-O_m))))*np.sinh(2*(np.arctanh(np.sqrt(abs(1-O_m)))-np.arctanh(np.sqrt(abs(1-O_m))/np.sqrt((O_m*(1+x))+ (1-O_m))))))
...
normO_mStndDev = round(O_mStndDev,4)
...
normchisquar = np.round((chisq/(N-P)),3)
...
plt.plot(x, yfit1, color="orange", label="2PArctanh model")
...
fig.savefig("2PArctan_least_squares_D_L_data.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("2PArctan_least_squares_D_L_data.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
