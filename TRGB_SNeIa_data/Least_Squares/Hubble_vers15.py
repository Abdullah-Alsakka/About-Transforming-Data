...
"""
print()
print("This curve_fit regression routine of Python scipy, uses the 'distance mag' data, converted to luminosity distances, D_L, vs redshift.") 
print("Data from Brout et al. 2022, 'The Pantheon+ Analysis: Cosmological Constraints' Astrophys. J. vol. 938, 110 and W. Freedman et al. 2019") 
print("The Carnegie-Chicago Hubble Program. VIII. ApJ vol. 882, 34. Here we use the Hubble model with only a single parameter, the Hubble constant;") 
print("no values are calculated for matter density, dark energy or space geometry. The Python 3 least_squares robust regression routine is employed")
print("with the specification that the routine specifies loss='cauchy' or 'linear' (w/o robust routine).")
...
#yfit1 now describes the R_h model
yfit1 = litesped*x/Hu
...
# where P is the parameter count, N the number of data pairs (1720), the r^2 the adjusted value rounded to 4 digits
P=1
N=1720
e = 2.718281
...
#We write a comma after StndDev, because we are extracting this value from tuple. Below is just rounding off the value of the standard deviation 
#to something one might believe to only 3 digits.
...
#calculate the statistical fitness, in the more usual manner, with 1720 as the number of data pairs and 1 as the degree of freedom (paramater count)
...
plt.plot(x, ydata, 'bo', label="SNe Ia + TRGB data")
plt.plot(x, yfit1, color="orange", label="Hubble model")
...
fig.savefig("Hubble_least_squares_D_L_data.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("Hubble_least_squares_D_L_data.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
