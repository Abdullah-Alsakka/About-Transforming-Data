...
"""
print()
print("This curve_fit regression routine of Python scipy, uses the data, as D_L vs redshift (z), from Brout et al. 2022, 'The Pantheon+ Analysis: Cosmological Constraints' Astrophys. J. vol. 938, 110 and the 19 TRGB data from Freedman et al. 2019 'The Carnegie-Chicago Hubble Program. VIII.' ApJ vol. 882, 34.  The model used is the LCDM integrated solution, the standard model of cosmology, with three parameters, the Hubble constant, Hu, and the normalized matter density (\u03A9m,O_m) and dark energy (\u03A9\u039B). The value for the normalized spacetime influence can be calculated from these results. The least_squares Python 3 regression is used specifying either the loss=linear or cauchy functions. Integration is necessary because the FLRW model cannot be solved exactly with mag vs. redshift z data.")
...
def func1(beta,x,ydata):
    return ydata - ((litesped*(1+x)/(np.sqrt(np.abs(1-beta[1]-beta[2]))*beta[0]))*np.sinh(np.sqrt(np.abs(1-beta[1]-beta[2]))*func2(x,beta[1],beta[2])))

x0 = np.array([70,0.33,0.66])#Initial guesses: Hubble constant value and normalized matter density: the least_squares regression routine used; x0, the parameter guesses; jac, the regression routine chosen; bounds, allowed lower, then upper bounds for params; loss, treatment of outlier data pairs; args, x and y data. 
result2 = least_squares(func1, x0, jac='3-point',bounds=((50,0.0001, 0.0001),(80,1.0, 1.0)),loss='cauchy',args=(x, ydata))

#extracting the Hu, O_m and O_L parameters and rounding the values
Hu,O_m,O_L = result2.x
Hubble = np.round(Hu,2)
mattdens = np.round(O_m,3)
DE = np.round(O_L,3)

#the yfit1 function used to estimate goodness of fit using the Hu, O_m and O_L estimates.
yfit1 = (litesped*(1+x)/(Hu*(np.sqrt(np.abs(1-O_m-O_L))))*np.sinh(np.sqrt(np.abs(1-O_m-O_L))*(func2(x,O_m,O_L))))
...
StndDev,O_mStndDev,DEStndDev = perr
normSD = round(StndDev,3)
normO_mStndDev = round(O_mStndDev,4)
norm_DEDev = round(DEStndDev,4)

#calculate the statistical fitness, according to astronomers, using 1720 as the number of data pairs and P=2 as the degree of freedom (paramater count)
chisq = sum(((ydata-yfit1)**2)/errors**2)
normchisquar = np.round((chisq/(N-P)),4)

#calculate the statistical usual fitness, using 1720 as the number of data pairs and 1 as the degree of freedom (paramater count).
...
plt.plot(x, yfit1, color="green", label="3Pstandard model")
...
print("The estimated \u03A9\u039B and S.D. are: ", DE, ",",  norm_DEDev)
print()
...
fig.savefig("3PLCDM_least_squares_eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("3PLCDM_least_squares_.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
