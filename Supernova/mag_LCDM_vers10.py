...
#open data file
with open("Gold_Riess2004.csv","r") as i:
...
#Extracting and rounding the estimated oarameter standard errors.
...
#normalised chisquar is calculated for 157 data pairs as  
normchisquar = round((chisquar/(157-2)),2)
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m), color = "green", label = "Flat $\Lambda$CDM model")
plt.xlabel("Redshift z", fontsize = 18)
plt.ylabel("mag (no units)", fontsize = 18)
#plt.title("$\Lambda$CDM flat model, mag vs. z data", fontsize = 18)
...
# fig.savefig("LambdaCDM_flat_mag_Z_data.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
# fig.savefig("LambdaCDM_flat_mag_Z_data.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
