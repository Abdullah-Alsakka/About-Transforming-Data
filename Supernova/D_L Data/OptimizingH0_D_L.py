...
We also massage the ydata below exp_fact=0.9 from 0.97 to 1.03 of the original value just to see
...
data_without_headerclutter = pd.read_csv("Gold_Riess_D_L_2004.csv")
DATA = data_without_headerclutter[['Exp_fact', 'D_L', 'err_D_L']]
DATA.drop(DATA.head(1).index,inplace=True)    # Drop Exp_fact = 1 to eliminate infinities
...
Hubble_range = np.linspace(60,62,20)
...
for des_Hu in Hubble_range:
    ...
    ## Curve Fitting ##    
    def func3(x,O_m):
        return (litesped/(x*des_Hu))*np.sinh(func2(x,O_m))

    def func4(x,O_m):
        return (litesped/(x*des_Hu))*np.sinh(integr(x,O_m))
    ...
    max_a = 0.9
    possible_y = np.empty([len(xdata[np.where(xdata < max_a)[0][0]:-1])+1,2*N+1])
    ...
    for j in range(len(xdata[np.where(xdata < max_a)])):
        a = len(xdata)-len(xdata[np.where(xdata < max_a)])+j
    ...
    new_ydata = []
    new_astroydata = []
    for i in range(len(xdata[np.where(xdata < max_a)])):
    ...
    mod_ydata = np.array(ydata[0:len(xdata)-len(xdata[np.where(xdata < max_a)])].tolist()+new_ydata)
    mod_astroydata = np.array(ydata[0:len(xdata)-len(xdata[np.where(xdata < max_a)])].tolist()+new_astroydata)
