...
"""
print()
print("This is the 3PsinmagStandard model, a version of the standard model of cosmology.")
print("The correlation is distance mag vs. redshift, z, with sinn(x) = sin(x) for hyperbolic geometry")
...
def func3(x,Hu,O_m,O_k):
    return 5*(np.log10((litesped*(1+x)/(np.sqrt(np.abs(O_k))*Hu))*(np.sqrt(np.abs(O_k))*np.sin(func2(x,O_m,O_k))))) + 25
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m,ans_O_k), color = "green", label = "3PsinmagStandard model")
...
fig.savefig("3PsinmagStandard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("3PsinmagStandard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
