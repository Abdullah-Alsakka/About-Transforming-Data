...
print("This curve_fit regression routine of Python scipy, uses the mag vs redshift (z) 
      ...
      with hyperbolic geometry, sinn(x) = sin(x).")
"""
print()
print("This is the sinmagStandard model of cosmology, but with sinn(x) = sin(x).")
print()
...
def func3(x,Hu,O_m):
    return 5*(np.log10((litesped*(1+x)/Hu)*np.sin((func2(x,O_m))))) + 25
...
# statistics are calculated for 1701 data pairs with P the parameter count (2) as
P=2
N=1701
e=2.71828183
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m), color = "green", label = "sinmagStandard model")
...
fig.savefig("sinmagStandard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("sinmagStandard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
