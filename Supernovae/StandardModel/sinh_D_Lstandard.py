...
The standard (LCDM) model used here with sinh requires numerical integration with two 
parameters, the Hubble constant, Hu and the normalised matter density, O_m with 
presumed elliptical space geometry. An estimate of the normalized cosmological 
constant (dark energy) is possible but nothing is uncovered about the geometry of 
space which is presumed elliptical.
"""
...
print()
print("This is the D_L_standard model of cosmology, but using sinn(x) = sinh(x).")
...
e = 2.718281
"""
#calculate the common statistical fitness, using 1702 as the number of data pairs and 2 as the paramater count.
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m), color = "orange", label = "sinh $D_L$standard model")
...
#print("The usual goodness of fit, \u03C7\u00b2, is: ", normchisquared)
print("The BIC estimate is: ",rBIC)
print()

#Saving the plots in two different formats
fig.savefig("sinh_D_L_standard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("sinh_D_L_standard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
