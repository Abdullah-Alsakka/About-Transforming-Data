...
"""
print()
print("This is the 3PsinD_Lstandard model. The correlation is luminosity distance, D_L vs. ")
print("expansion factor with sinn(x) = sin(x) for hyperbolic geometry.")
print("")
...
def func3(x,Hu,O_m,O_k):
    return (litesped/(x*Hu*np.sqrt(np.abs(O_k))))*(np.sqrt(np.abs(O_k)))*np.sin(func2(x,O_m,O_k))

init_guess = np.array([70,0.30,0.69])
bnds=([60,0.0001,0.001],[80,1.0,1.0])
