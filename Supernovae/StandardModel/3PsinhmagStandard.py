...
"""
print()
print("This is the sinh_3PmagStandard model, a version of the standard model of cosmology.")
print("The correlation is distance mag vs. redshift, z, with sin(x) = sinh(x) for elliptical geometry")
...
O_k = 0.69 #initial guess for Omega_L

# where t is the "dummy" variable for integration
def integr(x,O_m,O_k):
    return intg.quad(lambda t: 1/(((np.sqrt(((1+t)**3)*O_m + ((1+t)**2)*(O_k) + 1-O_m-O_k)))), 0, x)[0]
...
def func3(x,Hu,O_m,O_k):
    return 5*(np.log10((litesped*(1+x)/(np.sqrt(np.abs(O_k))*Hu))*(np.sqrt(np.abs(O_k))*np.sinh(func2(x,O_m,O_k))))) + 25

# guess for the Hubble constant, Hu, the matter density O_m and the space parameter, O_k.
init_guess = np.array([70,O_m,O_k])

# allowed range for the three parameters
bnds=([60,0.001,0.0001],[80,1.0,1.0])
...
rans_O_k = round(ans_O_k,4)
rans_O_L = round(1 - rans_O_m - rans_O_k,4)

# extracting and rounding the estimated standard deviations.
perr = np.sqrt(np.diag(pcov))
SD_Hu, SD_O_m, SD_O_k = perr
rSD_Hu = round(SD_Hu,2)
rSD_O_m = round(SD_O_m,3)
rSD_O_k = round(SD_O_k,3)

# calculating the value of O_L, S.D. and rounding off 
O_L_SD = np.sqrt((SD_O_m)**2 + (SD_O_k)**2)
rSD_O_L = round(O_L_SD,3)

# statistics are calculated for N=1701 data pairs with P the parameter count (3) as
P=3
N=1701
e=2.71828183
...
newxsqrd = sum(((ydata - func3(xdata,ans_Hu,ans_O_m,ans_O_k))**2)/(error**2))
newxsqrded = round(newxsqrd/(N-P),2)
"""
#Calculate the chi^2 as commonly done
chisqrd = sum(((ydata - func3(xdata,ans_Hu,ans_O_m,ans_O_k))**2)/(func3(xdata,ans_Hu,ans_O_m,ans_O_k)))
...
plt.plot(xdata, func3(xdata,ans_Hu,ans_O_m,ans_O_k), color = "green", label = "3PsinhmagStandard model")
...
fig.savefig("3PsinhmagStandard.eps", format="eps", dpi=2000, bbox_inches="tight", transparent=True)
fig.savefig("3PsinhmagStandard.pdf", format="pdf", dpi=2000, bbox_inches="tight", transparent=True)
